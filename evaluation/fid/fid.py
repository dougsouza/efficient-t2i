...
def _handle_path(path, sess):
    if path.suffix == '.npz':
        ...
        imgs = h5py.File(path, 'r')['samples']
        m, s = calculate_activation_statistics(imgs, sess)
        del imgs #clean up memory
    return m, s


def calculate_fid_given_paths(paths, inception_path=None):
...
def compute_train_stats(data_loader, dataset_name):
    ckpt_path = THIS_PATH / 'checkpoint'
    ckpt_path.mkdir(exist_ok=True)
    ckpt_path = check_or_download_inception(ckpt_path)
    create_inception_graph(str(ckpt_path))
    all_preds = []
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        inception_layer = _get_inception_layer(sess)
        for imgs in tqdm.tqdm(data_loader):
            assert type(imgs) == np.ndarray and imgs.dtype == np.float32 \
                   and imgs.shape[3] == 3
            preds = sess.run(inception_layer,
                        {'FID_Inception_Net/ExpandDims:0': imgs})
            preds = preds.reshape(len(imgs), -1)
            all_preds.append(preds)
    all_preds = np.vstack(all_preds)
    mu = np.mean(all_preds, axis=0)
    sigma = np.cov(all_preds, rowvar=False)
    np.savez(THIS_PATH / '{}_stats.npz'.format(dataset_name),
                        mu=mu, sigma=sigma)
